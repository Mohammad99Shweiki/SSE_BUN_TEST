...
export function sendEvent(client: SSEClient, event: string, data: unknown): boolean {
  if (client.closed || client.res.writableEnded) return false;
  try {
    client.res.write(encoder.encode(`event: ${event}\ndata: ${JSON.stringify(data)}\n\n`));
    return true;
  } catch {
    client.closed = true;
    return false;
...
export function broadcast(roomName: string, payload: BroadcastPayload): number {
  ...
  for (const clientId of roomClients) {
    const client = clients.get(clientId);
    if (!client || client.closed || client.res.writableEnded) continue;
    try {
      client.res.write(eventBytes);
      sentCount++;
    } catch {
      client.closed = true;
    }
