...
async function testMassConnections(): Promise<SSEConn[]> {
	...
	const receivePromises = conns.map((c) =>
...
async function testThroughput(existingConns: SSEConn[]): Promise<void> {
	...
	for (const conn of conns) {
		...
		if (!conn.listeners.has("entity-event"))
			conn.listeners.set("entity-event", []);
...
`);
	...
	console.log(
		`  ASSERT >50% delivery: ${deliveryOk ? "PASS" : "FAIL"} (${(avgDeliveriesPerMessage * 100).toFixed(1)}%)`,
	);
